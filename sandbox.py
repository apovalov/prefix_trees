    ...
    def add_query(self, query: str, value: float) -> None:
        ...
        for char in query:
            if char not in node.children:
                print(f"Создаем узел '{char}'")  # Отладочный вывод
                node.children[char] = Node()
            else:
                print(f"Переходим к существующему узлу '{char}'")  # Отладочный вывод
            node = node.children[char]
        node.is_end = True
        node.value = value
        print(f"Добавлена строка '{query}', значение: {value}")  # Отладочный вывод
        ...
        def find_node(node, prefix):
            for char in prefix:
                if char in node.children:
                    print(f"Переходим к узлу '{char}'")  # Отладочный вывод
                    node = node.children[char]
                else:
                    print(f"Узел '{char}' не найден")  # Отладочный вывод
                    return None
            return node

        def collect_suffixes(node, prefix, results):
            if node.is_end:
                print(f"Найден суффикс: {prefix}, значение: {node.value}")  # Отладочный вывод
                results.append((node.value, prefix))
            for char, child_node in node.children.items():
                collect_suffixes(child_node, prefix + char, results)

        print(f"Поиск суффиксов для префикса: {prefix}")  # Отладочный вывод
        results = []
...
class ReversedTrie(Trie):
    ...
    def add_query(self, query: str, value: float) -> None:
        ...
        print(f"Добавляем строку в обратном порядке: {reversed_query}")  # Отладочный вывод
    ...
    def prefixes(self, suffix: str) -> List[Tuple[float, str]]:
        ...
        print(f"Поиск префиксов для суффикса: {suffix}, обратный суффикс: {reversed_suffix}")  # Отладочный вывод
        suffixes = super().suffixes(reversed_suffix)

        return [(value, prefix[::-1]) for value, prefix in suffixes]


# # Дополнительная отладка метода prefixes в ReversedTrie
# rtrie = ReversedTrie()
# rtrie.add_query("test", 1.0)  # Добавляется как 'tset'
#
# print("Отладка метода prefixes в ReversedTrie для суффикса 'ts':")
# result = rtrie.prefixes("ts")  # Ищем префиксы для 'ts', что в обратном порядке 'st'
# print(f"Результат поиска префиксов: {result}")
...
class Suggester:
    ...
    def suggest_query(self, query: str) -> List[Tuple[float, str]]:
        ...
        """
        # suggestions = set(self.trie.suffixes(query))
        # suggestions.update((val, prefix) for val, prefix in self.reversed_trie.prefixes(query))
        # return sorted(suggestions, key=lambda x: -x[0])

        trie_suggestions = set(self.trie.suffixes(query))
        reversed_trie_prefixes = self.reversed_trie.prefixes(query)

        # Используем префиксы из ReversedTrie для поиска в Trie
        for _, prefix in reversed_trie_prefixes:
            trie_suggestions.update(self.trie.suffixes(prefix))

        # Комбинирование и сортировка результатов
        unique_suggestions = sorted(trie_suggestions, key=lambda x: -x[0])
...
#     return suggester
#
# suggester = sample_suggester()
#
# # print('print_trie')
# # print(suggester.reversed_trie.print_trie() )
#
# print('suggest_query')
# results = suggester.suggest_query("ple")
# print(results)
...
def suggest(query: str, k: int = 10):
    ...
    # Уникальные предложения, отсортированные по значению и взятие первых k элементов
    unique_suggestions = sorted(set(suggestions), key=lambda x: -x[0])[:k]
    return {"query": query, "suggestions": [s[1] for s in unique_suggestions]}
